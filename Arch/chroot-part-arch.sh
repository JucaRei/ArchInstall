...
mkdir -pv /mnt/{home,.snapshots,root,srv,usr/local,boot,var/log,var/opt,var/tmp,var/cache,var/swap}
mkdir -pv /mnt/var/lib/containers
mkdir -pv /mnt/var/lib/containers/storage/overlay
mkdir -pv /mnt/var/lib/pacman
mkdir -pv /mnt/var/lib/libvirt
...
mount -o $BTRFS_OPTS,subvol=@root /dev/sda4 /mnt/root
mount -o $BTRFS_OPTS,subvol=@home /dev/sda4 /mnt/home
mount -o $BTRFS_OPTS,subvol=@srv /dev/sda4 /mnt/srv
# mount -o $BTRFS_OPTS,subvol=@var /dev/sda4 /mnt/var
mount -o $BTRFS_OPTS,subvol=@pacman /dev/sda4 /mnt/var/lib/pacman
mount -o $BTRFS_OPTS,subvol=@libvirt /dev/sda4 /mnt/var/lib/libvirt
mount -o $BTRFS_OPTS,subvol=@lxd /dev/sda4 /mnt/var/lib/lxd
mount -o $BTRFS_OPTS,subvol=@snapshots /dev/sda4 /mnt/.snapshots
mount -o $BTRFS_OPTS,subvol=@usr_local /dev/sda4 /mnt/usr/local
mount -o $BTRFS_OPTS,subvol=@var_log /dev/sda4 /mnt/var/log
mount -o $BTRFS_OPTS,subvol=@var_opt /dev/sda4 /mnt/var/opt
...
mount -o $BTRFS_OPTS,subvol=@containers /dev/sda4 /mnt/var/lib/containers
mkdir -pv /mnt/var/lib/containers/storage/overlay
mount -o $BTRFS_OPTS,subvol=@overlay /dev/sda4 /mnt/var/lib/containers/storage/overlay
mount -t vfat -o defaults,noatime,nodiratime /dev/sda1 /mnt/boot
