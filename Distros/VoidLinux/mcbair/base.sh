...
mkfs.btrfs ${LINUX} -f -L "Voidlinux"

main="/dev/disk/by-label/Voidlinux"
sw="/dev/disk/by-label/SWAP"
ef="/dev/disk/by-label/EFI"
...
mount -o $BTRFS_OPTS2,subvol=@ ${main} /mnt
...
mount -o $BTRFS_OPTS,subvol=@home ${main} /mnt/home
# mount -o $BTRFS_OPTS,subvol=@home /dev/vda3 /dev/disk/by-label/Voidlinux
mount -o $BTRFS_OPTS,subvol=@snapshots ${main} /mnt/.snapshots
# mount -o $BTRFS_OPTS,subvol=@snapshots /dev/vda3 /mnt/.snapshots
mount -o $BTRFS_OPTS,subvol=@var_log ${main} /mnt/var/log
# mount -o $BTRFS_OPTS,subvol=@var_log /dev/vda3 /mnt/var/log
# mount -o $BTRFS_OPTS,subvol=@swap /dev/vda2 /mnt/var/swap
mount -o $BTRFS_OPTS,subvol=@var_cache_xbps ${main} /mnt/var/cache/xbps
# mount -o $BTRFS_OPTS,subvol=@var_cache_xbps /dev/vda3 /mnt/var/cache/xbps
mount -t vfat -o defaults,noatime,nodiratime ${ef} /mnt/boot/efi #grub
...
127.0.0.1 localhost
::1 localhost
127.0.1.1 zion.localdomain zion
EOF
...
# ROOTFS
LABEL="Voidlinux" /               btrfs rw,$BTRFS_OPTS2,subvol=@                   0 0
...
# EFI
LABEL="EFI"       /boot/efi       vfat defaults,noatime,nodiratime                 0 2
...
# chroot /mnt $XBPS_ARCH xbps-install -y base-system base-devel linux-firmware intel-ucode void-repo-debug void-repo-nonfree void-repo-multilib void-repo-multilib-nonfree linux-firmware-network acl-progs light kbdlight powertop arp-scan xev earlyoom opendoas base-devel zstd bash-completion minised nocache parallel util-linux bcache-tools necho starship linux-lts linux-lts-headers efivar dropbear neovim base-devel gummiboot ripgrep dust eza zoxide fzf xtools lm_sensors inxi lshw intel-ucode zsh alsa-utils vim git wget curl efibootmgr btrfs-progs nano ntfs-3g mtools dosfstools sysfsutils htop elogind dbus-elogind dbus-elogind-libs dbus-elogind-x11 vsv vpm polkit chrony neofetch dust duf lua bat glow bluez bluez-alsa sof-firmware xdg-user-dirs xdg-utils --yes
...
#File Management
chroot /mnt xbps-install -S gvfs gvfs-smb gvfs-mtp gvfs-afc avahi avahi-discover udisks2 samba --yes
...
GRUB_CMDLINE_LINUX_DEFAULT="quiet hid_apple.swap_opt_cmd=1 loglevel=0 console=tty2 udev.log_level=0 vt.global_cursor_default=0 mitigations=off nowatchdog intel_idle.max_cstate=1 cryptomgr.notests initcall_debug intel_iommu=on intel_iommu=igfx_off no_timer_check noreplace-smp page_alloc.shuffle=1 rcupdate.rcu_expedited=1 tsc=reliable zswap.enabled=1 zswap.compressor=lz4hc zswap.max_pool_percent=10 zswap.zpool=z3fold"
