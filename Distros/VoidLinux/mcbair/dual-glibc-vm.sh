...
chroot /mnt xbps-install -S powertop bash-completion xtools alsa-utils alsa-firmware alsa-plugins-ffmpeg alsa-plugins-jack alsa-plugins-samplerate alsa-plugins-speex alsa-plugins-pulseaudio lsscsi dialog git curl nano elogind vsv vpm chrony bluez bluez-alsa xdg-desktop-portal --yes
...
chroot /mnt usermod -aG wheel,floppy,audio,video,optical,kvm,lp,storage,cdrom,xbuilder,input,tty juca
...
chroot /mnt xbps-install -Syu lightdm lightdm-gtk3-greeter --yes

chroot /mnt sed -i 's/#greeter-session=example-gtk-gnome/greeter-session=lightdm-gtk-greeter/g' /etc/lightdm/lightdm.conf

chroot /mnt ln -sfrv /etc/sv/lightdm /var/service
