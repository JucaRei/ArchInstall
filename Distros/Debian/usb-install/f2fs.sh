...
sgdisk -n 0:0:10MiB /dev/sdb
sgdisk -n 0:0:250MiB /dev/sdb
...
# mkfs.ext4 -O "^has_journal" /dev/sdb3 -L "Linux" -F 
mkfs.f2fs -l LINUX -O extra_attr,inode_checksum,sb_checksum,compression /dev/sdb3 -f 
# mkfs.f2fs -l "Linux" -O inode_checksum,sb_checksum,compression /dev/sdb3 -f 
...
#EXT4
# mount -t ext4 -O defaults,noiversion,auto_da_alloc,noatime,errors=remount-ro,commit=20,inode_readahead_blks=32,delalloc,barrier=0 /dev/sdb3 /mnt
# mount -t ext4 -O defaults,data=writeback,noatime,errors=remount-ro,commit=20,barrier=0,discard /dev/sdb3 /mnt
#F2FS
# mount -O nobarrier,discard,compress_algorithm=zstd:6,extra_attr,compress_chksum,atgc,gc_merge,lazytime /dev/sdb3 /mnt
mount -O compress_algorithm=zstd:6,compress_chksum,atgc,gc_merge,lazytime /dev/sdb3 /mnt
mkdir -pv /mnt/boot
...
UEFI_UUID=$(blkid -s UUID -o value /dev/sdb2)
...
# F2FS
OPTS="compress_algorithm=zstd:6,compress_chksum,gc_merge,lazytime"
...
# Bios
chroot /mnt grub-install --target=i386-pc --recheck --boot-directory=/boot /dev/sdb
# EFI
...
# test with qemu
# sudo qemu-system-x86_64 -enable-kvm -m 8G -drive file=/dev/sdb,format=raw,media=disk