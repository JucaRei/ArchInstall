...
chroot /mnt apt install man-db gdisk mtools p7zip udisks2 unattended-upgrades --no-install-recommends -y
...
cat >/mnt/etc/modprobe.d/broadcom-sta-dkms.conf <<HEREDOC
...
mkdir -pv /mnt/etc/scripts
cat > /mnt/etc/scripts/wl-or-brcmsmac.sh<< HEREDOC
#!/bin/sh
...
HEREDOC
chmod +x /mnt/etc/scripts/wl-or-brcmsmac.sh

cat << EOF >> /etc/rc.local
/etc/scripts/wl-or-brcmsmac.sh  ## switch drivers
EOF
