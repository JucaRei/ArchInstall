...
mkfs.ext4 -O "^has_journal" ${DRIVE}2 -L "Linux" -F 
...
# BTRFS_SYSTEM="noatime,ssd,compress-force=zstd:3,space_cache=v2,commit=120,discard=async"
...
# mount -o $BTRFS_SYSTEM,subvol=@ ${DRIVE}2 /mnt
...
# mount -o $BTRFS_SYSTEM,subvol=@var_log ${DRIVE}2 /mnt/var/log
# mount -o $BTRFS_SYSTEM,subvol=@var_cache_apt ${DRIVE}2 /mnt/var/cache/apt
# # mount -o $BTRFS_OPTS,subvol=@swap ${DRIVE}2 /mnt/swap
# mount -t vfat -o defaults,noatime,nodiratime /dev/sda1 /mnt/boot/efi

mount -t ext4 -O defaults,data=writeback,noatime,errors=remount-ro,commit=60,barrier=0,discard ${DRIVE}2 /mnt
...
# debootstrap --variant=minbase --include=apt,apt-utils,extrepo,cpio,cron,zstd,ca-certificates,perl-openssl-defaults,sudo,neovim,initramfs-tools,console-setup,dosfstools,console-setup-linux,keyboard-configuration,debian-archive-keyring,locales,busybox,btrfs-progs,dmidecode,kmod,less,gdisk,gpgv,neovim,ncurses-base,netbase,procps,systemd,systemd-sysv,udev,ifupdown,init,iproute2,iputils-ping,bash,whiptail --arch amd64 bookwarm /mnt "http://debian.c3sl.ufpr.br/debian/ bookwarm contrib non-free"
...
OPTS="rw,noatime,data=writeback,barrier=0,commit=120,nobh"

touch /mnt/etc/fstab
cat <<EOF >/mnt/etc/fstab
LABEL="Linux"           /               ext4 $OPTS            0 2
EOF
...
### ROOTFS ###
# UUID=$ROOT_UUID   /               btrfs rw,$BTRFS_SYSTEM,subvol=@                         0 0
# LABEL="Debian"      /               btrfs rw,$BTRFS_SYSTEM,subvol=@                         0 0
...
# LABEL="Debian"      /var/log        btrfs rw,$BTRFS_SYSTEM,subvol=@var_log                  0 0
# UUID=$ROOT_UUID   /var/cache/apt  btrfs rw,$BTRFS_OPTS,subvol=@var_cache_apt            0 0
# LABEL="Debian"      /var/cache/apt  btrfs rw,$BTRFS_SYSTEM,subvol=@var_cache_apt            0 0
...
# LABEL="Grub"      /boot/efi       vfat noatime,nodiratime,umask=0077        0 2
...
chroot /mnt echo "America/Sao_Paulo" >/mnt/etc/timezone &&
    ...
    # localedef -i en_US -f UTF-8 en_US.UTF-8
    chroot /mnt apt update
    chroot /mnt apt install dropbear --yes

#############
## Network ##
#############

chroot /mnt apt install nftables net-tools gvfs gvfs-backends  \
    firmware-linux-nonfree firmware-linux-free network-manager iwd rfkill --no-install-recommends -y
...
###############
#
chroot /mnt apt install aptitude rsyslog manpages acpid hwinfo lshw dkms pciutils linux-image-amd64 linux-headers-amd64 fonts-firacode \
    debian-keyring git htop grub-pc wget unzip curl sysfsutils --no-install-recommends -y
