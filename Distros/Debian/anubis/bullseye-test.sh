...
debootstrap --verbose --variant=minbase --no-check-gpg --include=apt,dpkg,bash,zsh,cpio,cron,zstd,ca-certificates,perl-openssl-defaults,sudo,nano,tasksel,initramfs-tools,console-setup,dosfstools,console-setup-linux,keyboard-configuration,debian-archive-keyring,locales,busybox,btrfs-progs,dmidecode,kmod,less,gdisk,gpgv,ncurses-base,netbase,procps,udev,dbus-broker,ifupdown,init,iproute2,iputils-ping,whiptail --arch amd64 bullseye /mnt "http://debian.c3sl.ufpr.br/debian/ bullseye contrib non-free"
...
chroot /mnt apt install linux-image-5.10.0-32-amd64 linux-headers-5.10.0-32-amd64 -f -y
...
chroot /mnt apt install nftables firmware-linux-free network-manager iwd rfkill --no-install-recommends -y
...
chroot /mnt apt install alsa-utils pulseaudio pavucontrol --no-install-recommends -y

# Bluetooth
# chroot /mnt apt install bluetooth bluez bluez-tools rfkill pulseaudio-module-bluetooth --no-install-recommends -y
...
###############
#
chroot /mnt apt install rsyslog acpi acpid dkms btrfs-compsize \
