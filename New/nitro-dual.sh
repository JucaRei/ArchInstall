...
BTRFS_SYS="noatime,ssd,compress-force=zstd:14,space_cache=v2,commit=120,discard=async"
...
# p1: EFI (1GB), p2: Boot (1GB), p3: Btrfs Pool (300GB), p4: exFAT (resto)
...
GRUB_CMDLINE_LINUX="rhgb quiet intel_iommu=on iommu=pt i915.enable_psr=0 i915.enable_fbc=1 nvidia-drm.modeset=1 msr.allow_writes=on nowatchdog split_lock_detect=off"
...
# ==== KDE Plasma MÍNIMO + Bluetooth ====
# ==== KDE Plasma MÍNIMO + Suporte Wi-Fi Intel & Bluetooth ====
echo "🖥️ Instalando KDE Plasma mínimo, Drivers de Wi-Fi e Bluetooth..."
dnf5 install -y \
    zram-generator \
    sddm \
    plasma-desktop \
